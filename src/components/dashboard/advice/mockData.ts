...
export const mockAdviceItems: AdviceItem[] = [
  // Left column items
  {
    ...
    theme: 'Light',
    // Interesting always takes double height (244px)
  },
  {
    type: 'meta-item',
    id: 'sunday-brunch',
  ...
  },
  {
    ...
    images: [
      ...
      // Plus 86 more images as shown in the counter
    ],
    rating: '4.6',
    distance: '1.4 км',
...
/**
 * Get items for displaying in the exact Figma layout
 * Returns items arranged in the two-column layout as shown in mockup
 */
export function getFigmaLayoutItems(): {
  ...
  return {
    leftColumn: [
      mockAdviceItems[0], // Interesting (double height) - Гид по городу
      mockAdviceItems[1], // MetaItem - Воскресные бранчи
      mockAdviceItems[2], // MetaItem - Банкоматы
      mockAdviceItems[3], // Cover - Товары для ремонта
    ],
    rightColumn: [
      mockAdviceItems[4], // MetaItemAd - Xiaomi
      mockAdviceItems[5], // MetaItem - Школьная форма
      mockAdviceItems[6], // RD (double height) - Geraldine
      mockAdviceItems[7], // MetaItem - Все рубрики
    ],
  };
}

/**
 * Get all items in the order they should appear in a mixed layout
 * The AdviceSection component will handle the proper masonry arrangement
 */
export function getMixedLayoutItems(): AdviceItem[] {
  // Return all items - masonry layout will distribute them properly
  // The AdviceSection masonry algorithm places items in the shorter column
